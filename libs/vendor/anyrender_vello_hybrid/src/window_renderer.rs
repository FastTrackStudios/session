...
impl WindowRenderer for VelloHybridWindowRenderer {
    ...
    fn set_size(&mut self, width: u32, height: u32) {
        ...
        // FTS: the surface is checked against its own size, not the scene's.
        // A resize that lands while the renderer is still Pending updates the
        // scene but has no surface to resize; the next call then found the
        // scene already that size and skipped the surface, leaving it at the
        // size `resume` was handed. See UPSTREAM.md.
        if let RenderState::Active(active) = &mut self.render_state
            && (active.render_surface.config.width != width
                || active.render_surface.config.height != height)
        {
