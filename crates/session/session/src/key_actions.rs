...
pub trait KeyDaw: Tracks + Items + Transport + Send + Sync + 'static {}
impl<T> KeyDaw for T where T: Tracks + Items + Transport + Send + Sync + 'static {}
...
impl<D: KeyDaw> KeyActions for KeyActionsImpl<D> {
    ...
    fn bake_key_signatures(&self) -> DawResult<()> {
        Err(DawError::NotSupported(
            "baking needs the project-file round trip — see session::key::bake".into(),
        ))
    }
