...
fn a_setlist_opens_every_song_into_one_engine_and_switches_between_them() {
    ...
    let loud = song(root.path(), "Loud", 0.8, "Loud - Nobody\n90bpm 4/4 #F\nCount 1\nvs 2\n1 4\n");
    let quiet = song(root.path(), "Quiet", 0.2, "Quiet - Nobody\n120bpm 4/4 #G\nCount 1\nvs 2\n1 5\n");
    ...
    assert_eq!(session_daw::open::current_song().as_deref(), Some(setlist.songs[0].project.as_str()));
    ...
    assert_eq!(setlist.songs[1].saved.as_deref(), Some(quiet.with_extension("session").as_path()));
    ...
    assert!((loudest(0) - 0.8).abs() < 0.02, "Loud's own file: {}", loudest(0));
    assert!((loudest(1) - 0.2).abs() < 0.02, "Quiet's own file: {}", loudest(1));

    // Picking the second makes it current.
    let project = setlist.pick(1, 2.0).map(|s| s.project.clone()).expect("a pick");
    session_daw::open::switch_song(&project);
    assert_eq!(session_daw::open::current_song(), Some(project));
    assert!((setlist.songs[0].left_at - 2.0).abs() < 1e-9, "where Loud was left is kept");
}
