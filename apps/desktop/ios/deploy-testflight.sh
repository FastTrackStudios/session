...
# Which product to build+ship. Defaults to Session; override for Task (or
# any dx iOS app in the tree):
...
# `[application] name`, so `session-desktop` would ship as something like
# "SessionDesktop" on the device unless overridden. CFBundleDisplayName is
# what iOS actually shows, so set it explicitly — the macOS script does too.
...
# drops that traffic.
/usr/libexec/PlistBuddy -c "Add :NSLocalNetworkUsageDescription string 'Connects to your session engine on the local network.'" "$APP/Info.plist" 2>/dev/null || true
...
ICONS_DIR="${ICONS_DIR:-$ROOT/apps/desktop/ios/Assets.xcassets}"
if [ -d "$ICONS_DIR" ]; then
