...
#[test]
fn the_album_file_is_found_by_walking_up_from_the_project() {
    ...
    let found = find_album(&song).expect("the album file above the song");
    assert_eq!(found, album.join(ALBUM_FILE));
    // From the album directory itself, too.
    assert_eq!(
        find_album(&album).as_deref(),
        Some(album.join(ALBUM_FILE).as_path())
    );
}

#[test]
fn the_nearest_album_file_wins() {
    let tmp = tempfile::tempdir().expect("tempdir");
    ...
    fs::create_dir_all(&song).expect("mkdir");

    assert_eq!(find_album(&song), Some(inner.join(ALBUM_FILE)));
}

#[test]
fn a_project_with_no_album_file_above_it_has_none() {
    let tmp = tempfile::tempdir().expect("tempdir");
...
#[test]
fn the_active_profile_is_the_machine_setting() {
    let tmp = tempfile::tempdir().expect("tempdir");
    ...
    let (name, room) = studios.active().expect("the active profile loads");
    assert_eq!(name, "golden-room");
    assert_eq!(room.resolve_input("DI 3"), Resolved::Audio { channel: 2 });
}

#[test]
fn the_active_profile_defaults_to_the_hostname() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let studios = Studios::at(tmp.path());
    ...
    let (name, _) = studios.active().expect("this host's profile");
    assert_eq!(name, host);
}

#[test]
fn a_profile_that_does_not_parse_is_an_error_not_an_absence() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let studios = Studios::at(tmp.path());
    write(&studios.profile_path("broken"), "inputs (not a map)");
    let err = studios
        .load("broken")
        .expect_err("a broken profile is an error");
    assert!(err.to_string().contains("broken"), "{err}");
    // And a missing one is a distinct, named absence.
    assert!(studios.load("nowhere").expect_err("missing").is_missing());
}
