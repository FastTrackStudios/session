...
fn group_by_performer(facts: &[Fact], rows: &[Row], order: Option<&PerformerOrder<'_>>) -> Vec<Row> {
...
mod tests {
    ...
    fn the_injected_order_overrides_first_appearance() {
        ...
        let rows = resolve(&s, &facts, Surface::Mixer, None, Some(&reverse));
    ...
    fn arrangement_grouping_is_the_plain_walk() {
        ...
        let rows = resolve(&s, &guitars(), Surface::Mixer, None, None);
        assert!(
