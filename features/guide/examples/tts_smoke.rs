...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!("[smoke] ORT_DYLIB_PATH={:?}", std::env::var("ORT_DYLIB_PATH").ok());
    eprintln!("[smoke] voice={voice} exists={}", std::path::Path::new(&voice).exists());
    ...
    eprintln!("[smoke] model loaded; voice_id={}", session_guide::TtsRenderer::voice_id(&tts));
    eprintln!("[smoke] synthesizing 'Chorus'…");
    let audio = session_guide::TtsRenderer::render(&mut tts, "Chorus")?;
    eprintln!("[smoke] OK: {} samples @ {} Hz", audio.samples.len(), audio.sample_rate);
    Ok(())
}
